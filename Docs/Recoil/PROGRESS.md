...
> + **「回正扣减压枪量」** 7 个（`LyraRecoilTest.spec.cpp` 的 `Compensation.*`，本次 P11）。
> 散布特性的独立文档见 → **[12_SpreadInProfile.md](12_SpreadInProfile.md)**。
...
│   │   ├── LyraRecoilProfile.h/.cpp   【P1】【P8】【P11】手感配置资产（唯一数值来源；含 Recoil|RollShake、Recoil|Spread，【P11】bCompensationAwareRecovery）
│   │   ├── LyraRecoilState.h/.cpp     【P2/P4】【P8】【P11】FRecoilRuntimeState —— 纯算法层（【P13】含散布通道；【P11】压枪量测量 + 回正扣减 ComputeRecoveryTarget）
│   │   └── LyraRecoilDebug.h/.cpp     【P2/P5】【P8】【P11】CVar + 屏幕面板（压枪量一行 / 散布面板）+ Roll 波形面板 + 世界 DebugDraw + CSV 导出
│   ├── LyraRangedWeaponInstance.h/.cpp 【改】持有 RecoilProfile + RecoilState；Tick 驱动；相机修改器挂载（三轴）；【P11】SampleRecoilPlayerAim() 喂瞄准采样
